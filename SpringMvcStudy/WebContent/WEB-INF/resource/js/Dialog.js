...
var IMGFOLDERPATH = '/SpringMvcStudy/resource/images/popup/';//图片路径配置
...
var dollarE = {};
dollarE.dollarA = function(attr,ele) {
	ele = ele || this;
	ele = dollar(ele);
	return ele.getAttribute?ele.getAttribute(attr):null;
}
dollarE.getTopLevelWindow = function(){
	var pw = window;
	while(pw!=pw.parent){
		pw = pw.parent;
	}
...
dollarE.hide = function(ele) {
	ele = ele || this;
	ele = dollar(ele);
  ele.style.display = 'none';
}
dollarE.show = function(ele) {
	ele = ele || this;
	ele = dollar(ele);
  ele.style.display = '';
}
dollarE.visible = function(ele) {
	ele = ele || this;
	ele = dollar(ele);
...
Core.attachMethod = function(ele){
	...
	try{
		...
		for(var prop in dollarE){
			ele[prop] = win.dollarE[prop];
		}
	}catch(ex){
...
Dialog.prototype.show = function(){
	var pw = dollarE.getTopLevelWindow();
	var doc = pw.document;
...
if ( !this.ParentWindow ){
	...
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='dollar(\"_forTab_"+this.ID+"\").focus();'></a></td>");
	...
	var bgdiv = pw.dollar("_DialogBGDiv");
	if(!bgdiv){
		bgdiv = pw.document.createElement("div");
		bgdiv.id = "_DialogBGDiv";
		dollarE.hide(bgdiv);
	 	pw.dollarT("body")[0].appendChild(bgdiv);
		if(isIE6){
	...
	var div = pw.dollar("_DialogDiv_"+this.ID);
	if(!div){
		...
		//div.setAttribute("dragStart","Dialog.dragStart");
	 	pw.dollarT("body")[0].appendChild(div);
	}
	/*div.onmousedown = function(evt){
		var w = dollarE.getTopLevelWindow();
	...
	if(this.innerElementId){
		var innerElement=dollar(this.innerElementId);
		...
		if(isIE){
			...
			pw.dollar("_Covering_"+this.ID).parentNode.appendChild(fragment)
		}else{
			pw.dollar("_Covering_"+this.ID).parentNode.appendChild(innerElement)
		}
	}
	pw.dollar("_DialogDiv_"+this.ID).DialogInstance = this;
	if(this.URL)
		pw.dollar("_DialogFrame_"+this.ID).DialogInstance = this;
	pw.Drag.init(pw.dollar("_draghandle_"+this.ID),pw.dollar("_DialogDiv_"+this.ID));//注册拖拽方法
	if(!isIE){
		pw.dollar("_DialogDiv_"+this.ID).dialogId=this.ID;
		pw.dollar("_DialogDiv_"+this.ID).onDragStart = function(){dollar("_Covering_"+this.dialogId).style.display=""}
		pw.dollar("_DialogDiv_"+this.ID).onDragEnd = function(){dollar("_Covering_"+this.dialogId).style.display="none"}
	}

	this.OKButton = pw.dollar("_ButtonOK_"+this.ID);
	this.CancelButton = pw.dollar("_ButtonCancel_"+this.ID);

	//显示标题图片
	if(this.ShowMessageRow){
		dollarE.show(pw.dollar("_MessageRow_"+this.ID));
		if(this.MessageTitle){
			pw.dollar("_MessageTitle_"+this.ID).innerHTML = this.MessageTitle;
		}
		if(this.Message){
			pw.dollar("_Message_"+this.ID).innerHTML = this.Message;
		}
	}

	//显示按钮栏
	if(!this.ShowButtonRow){
		pw.dollar("_ButtonRow_"+this.ID).hide();
	}
	if(this.CancelEvent){
	...
	if(!this.AlertFlag){
		...
		//this.ParentWindow.dollarD = this;
	}
	this.OKButton.focus();
...
Dialog.prototype.close = function(){
	if(this.innerElementId){
		var innerElement=dollarE.getTopLevelWindow().dollar(this.innerElementId);
		...
		if(isIE){
			...
			fragment.innerHTML=innerElement.outerHTML;
			innerElement.outerHTML="";
			dollarT("body")[0].appendChild(fragment)
		}else{
			dollarT("body")[0].appendChild(innerElement)
		}

	}
	if(this.WindowFlag){
		this.ParentWindow.dollarD = null;
		this.ParentWindow.dollarDW = null;
		...
		if(this.AlertFlag){
			dollarE.hide(pw.dollar("_AlertBGDiv"));
		}
		if(!flag&&!this.AlertFlag){//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = dollarE.getTopLevelWindow();dollarE.hide(w.dollar("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id,txt,func){
	...
	var pw = dollarE.getTopLevelWindow();
	pw.dollar("_DialogButtons_"+this.ID).dollarT("input")[0].getParent("a").insertAdjacentHTML("beforeBegin",html);
	pw.dollar("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.getInstance = function(id){
	var pw = dollarE.getTopLevelWindow()
	var f = pw.dollar("_DialogDiv_"+id);
	if(!f){
		return null;
	}
...
Dialog.alert = function(msg,func,w,h){
	...
	diag.show();
	pw.dollar("_AlertBGDiv").style.display="";
	dollarE.hide(pw.dollar("_ButtonOK_"+diag.ID));
	var win = pw.dollar("_DialogFrame_"+diag.ID).contentWindow;
	...
	pw.dollar("_DialogButtons_"+diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg,func1,func2,w,h){
	...
	diag.show();
	pw.dollar("_AlertBGDiv").style.dispaly="";
	var win = pw.dollar("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
...
PageOnLoad=function (){
	...
	if(d){
		try{
			d.ParentWindow.dollarD = d;
			d.ParentWindow.dollarDW = d.Window;
			var flag = false;
			if(!this.AlertFlag){
				...
				if(!flag){
					dollarE.getTopLevelWindow().dollar("_DialogBGDiv").style.opacity="0.4";
					dollarE.getTopLevelWindow().dollar("_DialogBGDiv").style.filter="alpha(opacity=40)";
				}
			}
			if(d.AlertFlag){
				dollarE.show(dollarE.getTopLevelWindow().dollar("_AlertBGDiv"));
			}
			if(d.ShowButtonRow&&dollarE.visible(d.CancelButton)){
...
Dialog.setPosition=function(){
	...
	{
		pw.dollar("_DialogDiv_"+DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	//pw.dollar(this.bgdivID).style.width= sw + "px";
	pw.dollar(this.bgdivID).style.height= sh + "px";
}
